...
# kernel path: results/my_experiment/torchinductor_cache_0/ay/cayjv547275ntnosc2jdr5gzkqwg7xtefp24ubaot3iqk3aayinw.py
...
# kernel path: results/my_experiment/torchinductor_cache_0/vq/cvqdyozaghmi4lp4iyhx2fxobezicbtl7nf4ani35a2ls2sbifet.py
...
# kernel path: results/my_experiment/torchinductor_cache_0/nm/cnmj2ukfucerktmrjgpq35bs3oeksfhnui4p4hmciclsezlyt5t2.py
...
# kernel path: results/my_experiment/torchinductor_cache_0/gt/cgtrxlj3ca3vwgm3s4ztbgriuca2uox64puhggafptj34gkkwseu.py
# Topologically Sorted Source Nodes: [silu], Original ATen: [aten.silu]
# Source node to ATen node mapping:
#   silu => convert_element_type_13, convert_element_type_14, mul_7, sigmoid
# Graph fragment:
#   %convert_element_type_13 : [num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_8, torch.float32), kwargs = {})
#   %sigmoid : [num_users=1] = call_function[target=torch.ops.aten.sigmoid.default](args = (%convert_element_type_13,), kwargs = {})
#   %mul_7 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_13, %sigmoid), kwargs = {})
#   %convert_element_type_14 : [num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_7, torch.bfloat16), kwargs = {})
...
@triton_heuristics.pointwise(
    ...
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*bf16', 'out_ptr0': '*bf16', 'xnumel': 'i32', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='cuda', index=0, multi_processor_count=82, cc=86, major=8, regs_per_multiprocessor=65536, max_threads_per_multi_processor=1536, warp_size=32), 'constants': {}, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (2,): [['tt.divisibility', 16]]}]},
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_silu_6', 'mutated_arg_names': [], 'optimize_mem': False, 'no_x_dim': False, 'num_load': 1, 'num_reduction': 0, 'backend_hash': '6769567BF9CB28FC4140E0A692BA44D0F6558D1D18952489FF33DAF4A85D2E95', 'are_deterministic_algorithms_enabled': False, 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': False, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False},
    min_elem_per_thread=0
)
@triton.jit
...
def call(args):
    ...
    args.clear()
    assert_size_stride(primals_1, (2, 256, 1152), (294912, 1152, 1))
    ...
    assert_size_stride(primals_7, (2, 256, 1152), (294912, 1152, 1))
    assert_size_stride(primals_8, (2, 1152), (1152, 1))
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        buf0 = empty_strided_cuda((2, 256, 1152), (294912, 1152, 1), torch.bfloat16)
        # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy]
        stream0 = get_raw_stream(0)
        triton_poi_fused__to_copy_0.run(primals_1, buf0, 589824, stream=stream0)
        del primals_1
        ...
        del primals_3
        buf3 = empty_strided_cuda((512, 4608), (4608, 1), torch.bfloat16)
        # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.addmm]
        extern_kernels.addmm(buf2, reinterpret_tensor(buf0, (512, 1152), (1152, 1), 0), reinterpret_tensor(buf1, (1152, 4608), (1, 1152), 0), alpha=1, beta=1, out=buf3)
        del buf2
        buf4 = empty_strided_cuda((2, 256, 4608), (1179648, 4608, 1), torch.bfloat16)
        # Topologically Sorted Source Nodes: [gelu], Original ATen: [aten.gelu]
        stream0 = get_raw_stream(0)
        triton_poi_fused_gelu_3.run(buf3, buf4, 2359296, stream=stream0)
        ...
        extern_kernels.addmm(buf6, reinterpret_tensor(buf4, (512, 4608), (4608, 1), 0), reinterpret_tensor(buf5, (4608, 1152), (1, 4608), 0), alpha=1, beta=1, out=buf7)
        del buf6
        buf8 = empty_strided_cuda((2, 256, 1152), (294912, 1152, 1), torch.bfloat16)
        # Topologically Sorted Source Nodes: [mul, add], Original ATen: [aten.mul, aten.add]
        stream0 = get_raw_stream(0)
        triton_poi_fused_add_mul_5.run(primals_7, primals_6, buf7, buf8, 589824, stream=stream0)
        del primals_7
        buf9 = empty_strided_cuda((2, 1152), (1152, 1), torch.bfloat16)
        # Topologically Sorted Source Nodes: [silu], Original ATen: [aten.silu]
        stream0 = get_raw_stream(0)
        triton_poi_fused_silu_6.run(primals_8, buf9, 2304, stream=stream0)
    return (buf9, buf8, primals_6, primals_8, reinterpret_tensor(buf0, (512, 1152), (1152, 1), 0), buf3, reinterpret_tensor(buf4, (512, 4608), (4608, 1), 0), buf7, buf5, buf1, )


def benchmark_compiled_module(times=10, repeat=10):
    from torch._dynamo.testing import rand_strided
    from torch._inductor.utils import print_performance
    primals_1 = rand_strided((2, 256, 1152), (294912, 1152, 1), device='cuda:0', dtype=torch.float32)
    ...
    primals_7 = rand_strided((2, 256, 1152), (294912, 1152, 1), device='cuda:0', dtype=torch.bfloat16)
    primals_8 = rand_strided((2, 1152), (1152, 1), device='cuda:0', dtype=torch.bfloat16)
